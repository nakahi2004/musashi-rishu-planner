...
履修済みマーク・単位状況の反映が不要な場合は、Google Colabは不要です。

1. `rishu_planner.html` をブラウザ(Chrome, Edge, Safariなど)で開く
2. `Gakka/` フォルダ内の配布CSV（例：`mediashakai_courses.csv`）をドラッグ＆ドロップして読み込む
3. 履修計画を立てる

### ② Webアプリで開く（成績を反映する場合）

履修済みの授業のマークや単位取得状況を表示したい場合は、Google ColabでCSVを生成してから読み込みます。
...
| ファイル | 入手先 |
|---|---|
| 配布CSV（ご自身が所属する学科名のものを選択） | ダウンロードしたフォルダ内の `Gakka/` フォルダに入ってます（別途用意不要） |
