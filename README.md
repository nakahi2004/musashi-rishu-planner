...
[![Download](https://img.shields.io/badge/ダウンロード-v0.1.2_beta-6c63ff?style=for-the-badge)](https://github.com/nakahi2004/musashi-rishu-planner/releases/latest)
...
まずは試してみたい方へ：デモデータを利用し、使い勝手を確かめることができます。上の「ダウンロード」ボタンから最新のバージョンを選択。「Assets」内の「musashi-rishu-planner.zip」をダウンロードし、解凍してください。中に入ってる「rishu-planner.html」をダブルクリックし、ブラウザ(Chrome, Edge, Safariなど)で開きます※。表示されたWebアプリの「CSVファイルを選択」から、同じフォルダに入っている「demo.csv」を開いてください。
...
Webアプリに入力・利用するためのデータ(CSVファイル)を作ります。データの抽出のため、クラウド上でPythonを実行するGoogle Colabと、以下2つのPDFを利用します。
...
> **成績PDFをアップロードしたくない・用意できない(新1年生などの)場合**
> ステップ3をスキップしてステップ4を実行すると、履修済みマーク・単位状況なしの
> `merged_courses_no-personal-data.csv` が生成されます。履修済みの授業を非表示にしたり、履修の進捗状況をWebアプリ上で確認できません。

### ② Webアプリで開く

1. `rishu_planner.html` をブラウザ(Chrome, Edge, Safariなど)で開く
2. 生成した CSV (merged_courses.csv)をドラッグ＆ドロップ。または、ファイルをテキストエディタで開き、中身をコピー&ペーストして読み込む
3. 履修計画を立てる
